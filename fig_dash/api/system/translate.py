...
# google_trans_new author LuShan
# version : 1.1.9
import re
import json
import random
...
class DashTranslator:
    def __init__(self):
        self._trans_backend = google_translator()

    def __call__(self, text: str, target_lang: str="en"):
        """[summary]
        Translate given sentence to the `target_lang`

        Args:
            text (str): [description]
            target_lang (str, optional): [description]. Defaults to "en".

        Returns:
            [type]: [description]
        """
        return self._trans_backend.translate(text, lang_tgt=target_lang)


def test_translator():
    """[summary]
    Test the DashTranslator class. Currently it is just a wrapper around the google translator class.
    """
    trans = DashTranslator()
    translated_text = trans("this is a sentence", "hi")
