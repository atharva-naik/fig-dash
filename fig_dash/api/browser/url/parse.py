...
class UrlOrQuery:
    def __init__(self, url_or_query: str):
        ...
        if self.isUrl:
            self.protocol = parse_result[0]
            self.domain = parse_result[1].split("/")[0]
    ...
    def checkIfUrl(self, parse_result):
        domain = parse_result.netloc.split("/")[0].strip()
        ...
        #     try: self.result = list(parse.parse(fm_str, url_or_query))
        #     except TypeError: self.result = []
        #     if self.result != []: break
        # if self.result == []: return "query"
        # else: return "url"
